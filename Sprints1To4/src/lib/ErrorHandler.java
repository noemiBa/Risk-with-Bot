...
public class ErrorHandler {
    ...
    public String validatePlayerName(String playerName) {
        while (playerName.trim().isEmpty() || playerName.matches(".*\\d.*") || playerName.length() > 15) {
    ...
    public String validateDifferentName(ActivePlayer[] activePlayers, String playerName, Game risk) {
        while (activePlayers[1].getName().equals(activePlayers[0].getName())) {
            window.getTextDisplay("Looks like you guys have the same name, what a coincidence!\n" +
            ...
            try {
                ...
                if (player.getCountry(attackChoice[0]).getNumberOfUnits() == 1)
                    ...
                    window.sendErrorMessage("Sorry, you can only attack with at most " +
                            (player.getCountry(attackChoice[0]).getNumberOfUnits() <= 3 ? player.getCountry(attackChoice[0]).getNumberOfUnits() - 1
                                    : 3) + (player.getCountry(attackChoice[0]).getNumberOfUnits() == 1 ? " unit" : " units"));
                } else {
                    isValidChoice = true;
                }
            } catch (IllegalArgumentException e) {
    ...
    public int validateDicesDefend(Player player, String countryDefending) {
        ...
        if (player instanceof PassivePlayer) {
            ...
            window.getTextDisplay("You have " + numberOfUnit + " dice to defend with: ");
            if (numberOfUnit >= 2) {
